...
public class ArticleController {
    ...
    public String show(@PathVariable Long id, Model model){ //매개변수로 id 받아오기
       ...
       List<CommentDTO> commentsDtos = commentService.comments(id); // 댓글 목록 가져오기
        // 2. 모델에 데이터 등록하기
    ...
    @GetMapping("/articles/{id}/edit") // url요청접수
    ...
    public String upadte(ArticleForm form){ //매개변수로 DTO 받아오
        ...
        Article target = articleRepository.findById(articleEntity.getId()).orElse(null); // 데이터 찾기 = DB에서 데이터 찾기.데이터가 없으면 null반환
        // 2-2. 기존 데이터 값을 갱신하기
        if (target != null){
